...
/**
 * "Building your specific solution" loading screen for `/recommended-path`.
 *
 * The Server Component on the route awaits an Anthropic call that typically
 * runs 5-12 seconds. The earlier flat skeleton looked like a broken page
 * during that wait — this screen replaces it with a branded, animated
 * progress sequence that communicates work is being done on the user's
 * behalf.
 *
 * Pure CSS animations (defined in `globals.css`) so this stays an RSC and
 * does not need to ship a JS bundle to start animating. Each step exposes
 * a `--complete-at` custom property that drives both its own row-illumination
 * and the swap from spinner → check icon. The last step never auto-completes,
 * so a slow AI response never visually ends on "all done!" while the user is
 * still waiting.
 */
...
export default function RecommendedPathLoading() {
  return (
    <div
      className="flex min-h-screen flex-col items-center justify-center bg-dark-bg px-6 py-16"
      aria-busy="true"
      aria-live="polite"
    >
      ...
      {/* Pulsing PulseOne mark — concentric rings + steady centre dot. */}
      ...
      <p className="mb-10 max-w-[540px] text-center font-sans text-[15px] leading-relaxed text-white/55">
        We&rsquo;re tailoring radar signals, resources, and a recommendation to your situation.
          ...
          >
            <span className="relative inline-flex h-5 w-5 shrink-0 items-center justify-center">
              <span className="step-spinner absolute inset-0 flex items-center justify-center">
                <svg
                  className="loading-spinner h-5 w-5 text-pulse-teal"
                  viewBox="0 0 24 24"
                  fill="none"
                  aria-hidden
                >
              ...
              <span className="step-check absolute inset-0 flex items-center justify-center">
                <svg
                  className="h-5 w-5 text-pulse-teal"
                  viewBox="0 0 24 24"
                  fill="none"
                  aria-hidden
                >
      ...
      <div className="mt-10 h-1 w-full max-w-[460px] overflow-hidden rounded-full bg-white/10">
        <div className="loading-bar h-full rounded-full bg-gradient-to-r from-pulse-red to-pulse-teal" />
      </div>

      <p className="mt-6 max-w-[460px] text-center font-sans text-[12px] leading-relaxed text-white/35">
        Synthesis is generated by AI grounded in PulseOne&rsquo;s live radar — your specific
