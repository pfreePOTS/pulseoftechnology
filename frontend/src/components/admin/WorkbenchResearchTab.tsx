...
export default function WorkbenchResearchTab() {
  const [activeTab, setActiveTab] = useState<SubTab>("watched");
  ...
  /** Pending topic id while POST /watch is in flight */
  ...
  async function handleWatch(topicId: number) {
    setWatchingId(topicId);
    try {
      const r = await adminFetch(`${API_BASE}/api/admin/topics/${topicId}/watch`, {
        method: "POST",
      });
      if (r.ok) {
        await reloadTopics();
    ...
    <>
      <div className="max-w-5xl">
        ...
        <p className="mt-1 text-sm text-gray-400">
          Review discovered topics, merge duplicates, and decide what to watch. Watched topics are tracked for trends,
          impact analysis, and newsletter inclusion.
        </p>
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-gray-800 bg-gray-900 px-6 py-16 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {activeTab === "pending"
                ? "Run RSS ingestion to discover new topics. Watch the ones you want to track."
                : "Watch topics from the Pending tab to track them here."}
            </p>
          </div>
        ) : (
                    ...
                    <td className="px-4 py-3 text-right">
                      {activeTab === "watched" ? (
                        <Link
                          ...
                          className="inline-flex items-center justify-center rounded-md bg-gray-600 p-2 text-white transition-colors hover:bg-gray-500"
                        >
                        ...
                        <div className="flex items-center justify-end gap-2">
                          <button
                            type="button"
                            disabled={watchingId === topic.id}
                            onClick={() => void handleWatch(topic.id)}
                            className="rounded-md bg-indigo-600 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-indigo-500 disabled:opacity-60"
                          >
                            {watchingId === topic.id ? "Watching…" : "Watch"}
                          </button>
                          <Link
                            href={`/admin/topics/${topic.id}`}
                            title="Edit"
