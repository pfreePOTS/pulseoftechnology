...
1. **Refactor ****`backend/services/ai_service.py`**Break down the `evaluate_article` and `process_raw_articles` logic into a multi-step pipeline (graph) using separate prompts and models for each step:
...
1. **Update ****`process_raw_articles`**** Integration**
...
1. Verify in the logs that the multi-step graph executes sequentially.

1. Check the Curation Dashboard to confirm that articles are more accurately clustered into existing topics rather than fragmented.
